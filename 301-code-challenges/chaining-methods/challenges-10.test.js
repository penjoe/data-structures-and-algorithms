...
const count = (target, input) => {
  let result = 0;
  input.map( idx => idx.map( idx => idx === target ? result++ : result + 0))
  return result;
};
...
const totalSum = (input) => {
  let foo = input.map( idx => idx.reduce( (acc, idx) => acc + idx));
  return foo.reduce( (acc, idx) => acc+ idx);
};
...
const divisibleByFiveTwoToThePower = (input) => input.map( idx => idx.filter( num => num % 5 === 0 && Number.isInteger(num)).map( idx => Math.pow(2, idx)));
...
let findMaleAndFemale = (data) => data.filter( idx => idx.gender == 'male' || idx.gender === 'female').map( idx => idx.name).join(' and ');
...
// not happy with this answer. couldn't figure out what to use as filter element
let findShortest = (data) => data.map( idx => ({name: idx.name, height: idx.height})).filter( idx => idx.height < 100).map( idx => idx.name).join();
