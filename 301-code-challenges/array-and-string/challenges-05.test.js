...
const listFoods = (recipe) => {
  let result = [];
  let foo = recipe.ingredients;
  for (let i = 0; i < foo.length; i++){
    result.push(foo[i].slice(foo[i].indexOf(' ', foo[i].indexOf(' ')+1)+1))
  }
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  let foo = recipe.steps;
  for (let i = 0; i < foo.length; i++){
    result.push(foo[i].replace(/ .*/, ''))
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  // Solution code here...
  let foo = numberOfCharacters;
  if (foo > str.length) {
    return '';
  } else if (foo < 0) {
    return str;
  } else {
    return str.slice(0, str.length-foo);
  }
};
...
const extractVowels = (str) => {
  // Solution code here...
  let result = [];
  let foo = str.split(/[aeiou]/gi);
  let consonants = foo.join('');

  let bar = str.split(/[^aeiou]/gi);
  let unorderedVowels = bar.join('')
  let vowelArray = unorderedVowels.split('');
  let sortedVowels = vowelArray.sort();
  let vowelString = sortedVowels.join('');
  let vowels = vowelString.replace(/\s+/g, '');

  result.push(consonants, vowels);
  return result; 
};
