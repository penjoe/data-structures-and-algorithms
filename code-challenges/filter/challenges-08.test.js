...
const oddValues = (arr) => arr.filter( value => value % 2 === 1);
...
const filterStringsWithVowels = arr => arr.filter(idx => idx.match(/[aeiou]/g));
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter(idx => !forbiddenValues.includes(idx));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter( idx => idx.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) => arr.filter( idx => idx.baseStat > minBaseStat).map( item => item.stat.name);
...
const getCharactersWithoutChildren = (arr) => arr.filter( idx => !idx.children);
...
const evenOddNumericValues = (arr) => arr.filter(idx => Number.isInteger((idx))).map( item => item % 2 === 0 ? 'even':'odd');
