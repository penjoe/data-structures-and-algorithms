...
const forEachTwoToThe = (arr) => {
  let result = [];
  arr.forEach( idx => result.push(Math.pow(2, idx)));
  return result;
};
...
const mapTwoToThe = (arr) => arr.map( idx => Math.pow(2, idx));
...
const charCode = (arr) => arr.map(idx => idx.charCodeAt());
...
const extractAbilities = (arr) => arr.map( idx => idx.ability.name);
